...
- [ ] `New Lite`에서 단노트를 1개씩 칠 때 타격 hitch가 체감되지 않는지 확인
- [ ] `New Full`과 `New Lite`를 각각 같은 채보에서 비교해 Lite가 더 무거워지지 않는지 확인
- [ ] 2초/8초/20초/60초 롱노트 등장 시 프레임 드랍이 튀지 않는지 확인
- [ ] Canvas 2D와 WebGL 렌더러에서 롱노트 head/body/tail 위치가 같은지 확인
- [ ] 중앙 판정 텍스트와 레인별 FAST/SLOW 표시가 서로 밀어내지 않는지 확인
...
### 7.3 에디터 입력 안정성
- [ ] 좌우 패널의 작업 버튼 클릭 후 추가 클릭 없이 노트 배치가 가능한지 확인
- [ ] 슬라이더 조작 후 타임라인 단축키가 먹통이 되지 않는지 확인
- [ ] 입력 필드 밖에서 Ctrl+Z/C/Space 단축키가 정상 작동하는지 확인
- [ ] 숫자 입력, 파일 선택, 모달 내부 버튼은 focus가 필요한 동안 유지되는지 확인
- [ ] 재생/일시정지 빠른 반복, 현재 위치 시작, 스크롤바 드래그, Shift+드래그 추가 선택이 정상인지 확인
